...
import (
	...
	auction_srv "gateway/proto-go/auction_srv"
)
...
func (s *AuctionService) CreateAuction(itemName string, start, end uint64) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	_, err := s.client.CreateAuction(ctx, req)
	if err != nil {
		return false, fmt.Errorf("grpc CreateAuction failed: %w", err)
	}
